...
**注意**: 次のファイルは **.gitignore に含まれており、Git にコミットしません。**  
- **GoogleService-Info.plist**（API キー）  
- **\*.p8**（APNs 認証キー。例: AuthKey_xxxxx.p8）  
- **\*.key**（証明書の秘密鍵。例: AppleDistribution.key）  

これらが GitHub に含まれると、Google・GitHub・GitGuardian などから警告が届きます。クローンした人は、各自で Firebase Console や Apple Developer から必要なファイルを取得して追加してください。
...
1. **Google Cloud Console** でキーを無効化・再発行する  
   ...
   - 一覧から **API キー**（メールに記載のキー、例: 末尾が …K57lK8I）の行をクリックして開く。  
   - 画面内の **「キーを再生成」** または **「鍵をローテーション」**（Rotate key）のボタンをクリックする。  
   - 確認が出たら実行すると、**新しいキーが表示され、古いキーはその場で無効**になります。表示された新しいキーを控えておくか、次の手順で Firebase から plist を再ダウンロードします。
...
## 1.3. APNs キー（.p8）をローテーションする（漏洩した場合）

**.p8 が GitHub に漏れたとき**は、**新しい APNs キーを発行し、Firebase でその新キーだけを使う**ようにします。Apple では既存の .p8 を「無効化」する機能はなく、**新規キーを作成して切り替える**形になります。

### 手順 1：Apple Developer で新しい APNs キーを作る

1. [Apple Developer](https://developer.apple.com/account/) にログインする。
2. **Certificates, Identifiers & Profiles** を開く。
3. 左メニューで **Keys** をクリックする。
4. **+**（新規キーを作成）をクリックする。
5. **Key Name** に名前を付ける（例: ChildGuard APNs 2、または日付入りで区別しやすくする）。
6. **Apple Push Notifications service (APNs)** にチェックを入れる。
7. **Continue** → **Register** で作成する。
8. 表示された画面で **Download** をクリックし、**新しい .p8 ファイル** をダウンロードする。  
   **重要**: .p8 は **一度しかダウンロードできません**。安全な場所に保存し、**Git にコミット・プッシュしない**でください（.gitignore で `*.p8` は除外済み）。
9. **Key ID**（例: 2ZG9HZQJDL のような英数字）をメモする。Firebase に入力するときに使います。

### 手順 2：新しいキーを App ID に紐づける

1. Keys 一覧に戻り、**今作成したキー**の行の **Configure** をクリックする。
2. **App ID** で **ChildGuard**（Bundle ID: com.yoshi.ChildGuard）を選択する。
3. **Save** で保存する。

### 手順 3：Firebase に新しい .p8 を登録する

1. [Firebase Console](https://console.firebase.google.com/) でプロジェクト（ChildGuard）を開く。
2. 左メニュー **Build** → **Cloud Messaging** を開く。
3. 下にスクロールして **Apple アプリ設定** を開く。
4. 登録済みの **iOS アプリ（ChildGuard）** の行で、**アップロード** または **鍵アイコン／設定** をクリックする。
5. **APNs 認証キー** を選び、次を入力・アップロードする：
   - **APNs 認証キー (.p8 ファイル)**: 手順 1 でダウンロードした **新しい** .p8 をアップロードする。
   - **キー ID**: 手順 1 でメモした **新しい Key ID** を入力する。
   - **チーム ID**: Apple Developer の **Membership** に表示されている Team ID（例: F5558YDHK6）。
   - **Bundle ID**: `com.yoshi.ChildGuard`。
6. **アップロード**（または保存）をクリックする。

これで **新しい .p8 だけ** が FCM で使われ、漏れた古いキーは使われません。古い .p8 ファイルは手元から削除するか、別フォルダに退避して、リポジトリには絶対に含めないでください。
