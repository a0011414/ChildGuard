...
**エラー15** は CloudKit の「サーバーがリクエストを拒否」です。環境によっては解消しないことがあります。**その場合はアプリ内の「ルールをQRで表示」を使うと、CloudKit なしで確実に子にルールを渡せます。** 子の端末で URL スキーム `childguard` を有効にすれば、QR 読み取りでルールが反映されます（下記「ルールをQRで表示」の節参照）。
