...
1. **Bundle ID は 1 申請につき 1 つ**  
   メインアプリと App Extension で別 Bundle ID の場合は、**それぞれ別々に 1 件ずつ申請**する必要があります。フォームに「メインと Extension をまとめて」と書いても受け付けられないことがあるため、**2 件目は Extension 専用**で送ります。

   **Extension 用の 2 件目の申請のやり方**  
   - 同じ URL（https://developer.apple.com/contact/request/family-controls-distribution）を開き、**もう 1 回**フォームを送信する。
   - **App Apple ID** は、Extension には存在しないので **メインアプリの ChildGuard の App Apple ID（数字）をそのまま**入れる。
   - **Bundle ID** を聞かれる欄があれば **`com.yoshi.ChildGuard.ChildGuardDeviceActivityMonitorExtension`** とだけ書く。
   - アプリの説明欄や備考に、「ChildGuard の Device Activity Monitor Extension 用です。Bundle ID: com.yoshi.ChildGuard.ChildGuardDeviceActivityMonitorExtension」のように一言書いておく。
   - 目的・使い方は 1 件目と同じ（「家庭内のペアレンタルコントロール。DeviceActivity / ManagedSettings で使用時間の監視と制限」など）でよい。
