...
**✅ 申請済み**（審査結果待ち。目安 3 週間〜1 ヶ月。承認されたら Xcode で Family Controls を有効化し実装へ。）
...
- 承認されたら、Xcode の **Signing & Capabilities** に **Family Controls** を追加し、**DeviceActivity** / **ManagedSettings** の実装を進める。
- 最初の 1 ルール: 「〇時間超えそうなので止めた」→ 親に通知 → 自動で使用制限（TODO.md の「のちに Family Controls 申請後…」に対応）。
